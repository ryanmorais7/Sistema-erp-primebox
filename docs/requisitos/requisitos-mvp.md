...
| Impressão de pedido | Implementado | Tela de visualização (`/pedidos/[id]`) com botão "Imprimir" (CSS de impressão do navegador, sem geração de PDF no servidor). Inclui comprovante de recebimento em 2 vias (PrimeBox/Cliente) com campo pra nome, assinatura e data — ver ADR-023 |
