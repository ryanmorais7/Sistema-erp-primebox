...
|---|---|---|
| Cadastro de Clientes | Implementado e validado em produção | Listagem (`/clientes`), criação (`/clientes/novo`) e edição (`/clientes/[id]/editar`), com ativar/desativar (soft delete) e excluir de verdade (bloqueado se o cliente tiver pedido vinculado — ver ADR-029). Validação com Zod (exige CNPJ ou CPF, formatos de telefone/CEP/UF) e React Hook Form |
| Cadastro de Produtos | Implementado e validado em produção | Listagem (`/produtos`), criação (`/produtos/novo`) e edição (`/produtos/[id]/editar`), com ativar/desativar (bloqueado por FK contra exclusão de verdade, nenhuma ação de excluir chegou a ser implementada — ver ADR-029/ADR-035). Tipo (Base, Unibox ou Baú — catálogo real da fábrica, ver ADR-015) e medida via seleção (medida vem da tabela `Medida`); preço aceita formato brasileiro (ex: 1.899,90). Campo de custo de produção com margem estimada calculada ao vivo; coluna "Margem" na listagem. Catálogo esvaziado (todos inativados em massa) em 2026-08-19 — ver ADR-035 |
| Pedido | Implementado e validado localmente | Listagem (`/pedidos`), criação (`/pedidos/novo`) e edição (`/pedidos/[id]/editar`) com itens dinâmicos (cliente + produto + quantidade + preço unitário editável + custo unitário editável). Combobox de busca por nome do cliente. Campo de observações (anotações de pagamento). Ações de "Marcar como pago" (rótulo do botão, era "Faturar") e excluir (só para pedidos em carteira); pedido pago redireciona para a visualização. Status interno continua `EM_CARTEIRA`/`FATURADO` no banco — só o rótulo em tela virou "Em carteira"/"Pago" (badge teal), sem migration — ver ADR-036. Valor total sempre recalculado no servidor a partir dos preços informados. Campo Produto é texto 100% livre, sem autocomplete/sugestão do catálogo — salvo e exibido exatamente como digitado, sem preenchimento automático de preço/custo — ver ADR-035. Campos de valor (Preço unit./Custo para produzir) formatam sozinhos pro padrão brasileiro ao sair do campo — digitar "100" vira "100,00" sem precisar da vírgula, mesmo comportamento em todo campo de valor do sistema (Produção, Estoque, Produtos) — ver ADR-037. Rótulo "Custo unit." renomeado pra "Custo para produzir" em 2026-08-22 (formulário e tela de visualização do pedido). Bloco "Forma de pagamento" reposicionado pra logo abaixo de Cliente (antes de Itens) e ordem de Tab dentro de cada linha de item corrigida (Produto → Qtd. → Preço unit. → Custo para produzir, com foco automático no Produto da linha nova ao adicionar item), também em 2026-08-22 — ver ADR-009 |
...
| Gráfico de pagamentos no Painel | Implementado e validado localmente | Card no Painel (`/`) com barras dos últimos 6 meses (Recharts), agrupado por `updatedAt` do pedido pago. Clicar num mês filtra os 4 cards de resumo ("Em carteira", "Pedidos Pagos", "Clientes ativos", "Valor pendente") pra aquele mês, e mostra um card "Pago em {mês}" com variação vs. o mês anterior — ver ADR-019. Meses sem pagamento aparecem zerados, não somem. Cards renomeados e recalculados em 2026-08-20 (OP avulsa concluída/aguardando com valor > 0 soma junto com Pedido pago/em carteira) — ver ADR-036. "Pago em {mês}" e "Valor pendente" trocaram de posição em 2026-08-22 ("Pago em {mês}" subiu pra linha dos 4 cards, "Valor pendente" desceu pro card largo abaixo do gráfico) — ver ADR-036 |
...
- Ver [ADR-036](../decisoes/ADR-036-op-concluida-paga-painel-recibo.md) — Pedido "Pago" (rótulo), OP avulsa concluída conta como paga no Painel, cards recalculados, recibo mais denso.
- Ver [ADR-037](../decisoes/ADR-037-formatacao-campos-de-valor.md) — todos os campos de valor (R$) do sistema formatam sozinhos no blur, sem exigir vírgula/centavos digitados.
